...
class Component(Module):
    ...
    def ban_user(self, uid, client):
        ...
        for tmp in self.server.online.copy():
            ...
            break
        client.send(["cp.ms.rsm", {"txt": f" {uid} İDLİ KULLANICI BANLANMIŞTIR"}])
    ...
    def unban_user(self, uid, client):
        ...
        banned = redis.get(f"uid:{uid}:banned")
        if not banned:
            client.send(["cp.ms.rsm", {"txt": f" {uid} İDLİ KULLANICININ BANI KALDIRILMIŞTIR"}])
            return
    ...
    def mute_player(self, msg, client):
        ...
        for tmp in self.server.online.copy():
            ...
            tmp.send(["cp.m.bccu", {"bcu": {"notes": "", "reviewerId": "0",
                                            ...
                                            "categoryId": 14}}])
            break
        client.send(["cp.ms.rsm", {"txt": f" {apprnc['n']} ADLI KULLANICIYA "
                                          f" {minutes} DAKİKA KONUŞAMAMA CEZASI VERİLMİŞTİR"}])
    ...
    def reset_user(self, uid, client):
        ...
        client.send(["cp.ms.rsm", {"txt": f" {uid} İDLİ HESAP RESETLENMİŞTİR"}])
